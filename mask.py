...
class Mask:
    ...
    def load_table(self):
        ...
        Verify if information of colors is required for
        ...
        if self.table_name.split('.')[-1] in ('csv', 'txt'):
            ...
            self.table = pd.read_excel(self.table_name, engine='openpyxl')
        else:
            raise Exception(
        ...
        if self.table['Element'].str.contains('ouleur|olor').any():
            ...
            # Creation of dictionary containing the colors
            self.colors = {}
    ...
    def save_mask(self, indice: str, raw: bool = False):
        ...
        if not raw:
            ...
            image.save('Mask_' + self.Minerals[indice] + '.tif')
                                           ...
                                           #'''Debut: Partie2 modifiee '''#


    # Ajoutez une variable de classe pour suivre l'état de la fenêtre contextuelle d'histogramme
    histogram_popup = None
    def get_hist(self, indice: str, cadre5):
        """
        ...
        # Create the figure and axes for the elemental map
        fig_map = Figure(figsize=(4, 4), dpi=80)
        ax_map = fig_map.add_subplot(111)

        # Plot the elemental map
        im = ax_map.imshow(self.data_cube[:, :, indice], aspect='auto')
        ax_map.grid()
        ax_map.set_title("Elemental Map: " + self.Elements[indice])
        fig_map.colorbar(im, ax=ax_map)

        # Embed the elemental map in the existing cadre5 frame
        if cadre5.children:
            for child in cadre5.winfo_children():
                child.destroy()

        # Create a FigureCanvasTkAgg object with the figure
        embedded_map = FigureCanvasTkAgg(fig_map, master=cadre5)
        embedded_map.draw()
        embedded_map.get_tk_widget().pack(side="top", fill="both", expand=True)
        ...
        def show_histogram():  
            ...
            # Create a frame inside the popup window
            frame_histogram = Frame(popup)
            frame_histogram.pack(side="top")
            # Create the figure and axes for the histogram
            fig_hist = Figure(figsize=(4, 4), dpi=80)
            ax_hist = fig_hist.add_subplot(111)
            # Keep only finite values
            finite_data = self.data_cube[:, :, indice][np.isfinite(self.data_cube[:, :, indice])]

            # Plot the histogram
            hist_plot = sns.distplot(finite_data, kde=False, ax=ax_hist, hist_kws={'range': (0.0, np.max(finite_data))})
            ax_hist.set_yscale('log')
            ax_hist.set_title("Histogram of Intensity: " + self.Elements[indice])

            # Embed the histogram in the frame_histogram
            embedded_hist = FigureCanvasTkAgg(fig_hist, master=frame_histogram)
            embedded_hist.draw()
            embedded_hist.get_tk_widget().pack(side="top", fill="both", expand=True)
            fig_hist.tight_layout()

            # Mettez à jour la variable de classe avec la référence à la nouvelle fenêtre contextuelle
            self.histogram_popup = popup
            # Display the popup window
            popup.mainloop()

        # Create a button to show the histogram 
        histogram_button = Button(cadre5, text="Show Histogram", command=show_histogram)
        histogram_button.pack(side="top", fill="x")   
        # Créer la barre d'outils de matplotlib
        toolbar = NavigationToolbar2Tk(embedded_map, cadre5)
        toolbar.pack(side="bottom", fill="x")

   

                                        #'''FIN: Partie2 modifiee '''#
    ...
    def plot_mineral_mask(self, cadre8):
        # Déclarer la variable globale
        global interpolated_array
        self.interpolated_array = None
        ...
        canvas = FigureCanvasTkAgg(fig, cadre8)
        canvas.get_tk_widget().grid(row=0, column=0, sticky='nsew')
        canvas.draw()
        cadre8.grid_propagate(False)
        cadre8.grid_rowconfigure(0, weight=1)
        cadre8.grid_columnconfigure(0, weight=1)
     
        def perform_interpolation():
            # Mettre à jour le tracé avec l'interpolation "none"
            im.set_interpolation("none")  # Définir l'interpolation sur "none"
            fig.canvas.draw()

        # Créer un bouton pour effectuer l'interpolation
        interpolation_button = Button(cadre8, text="None interpolation", command=perform_interpolation)
        interpolation_button.grid(row=1, column=0)
