...
def open_file():
    global filenames, df, cadre7,cadre4
...
#Ajouter le scrolle pour le cadre4 pour pouvoir scroller les noms des images
canvas = tk.Canvas(cadre4, borderwidth=2, relief="solid", width=375, height=275)
canvas.grid(row=0, column=0, sticky="nsew")
scrollbar = Scrollbar(cadre4, orient="vertical", command=canvas.yview)
...
def update_filelist():
    ...
    for i in range(start_index, len(filenames)):
        ...
        if os.path.exists(os.path.join('images', filename)):
            button = Radiobutton(frame, text=filename, font=("Arial", 12), variable=selected_file, value=filename, command=lambda fname=filename: on_file_selection(fname))
            ...
            if column >= num_columns:
                ...
                if row >= 100 or i == len(filenames) - 1:
                    break
...
#Fonction qui permet d'ouvrir le tableau excel dans le cadre7 
def open_fileMask():
    global excel_file_path ,df
    ...
    fichierExcel=basename(filename)
    if filename:
        # Si un fichier est sélectionné, afficher son contenu dans le cadre7
        df = pd.read_excel(filename)
        cadre7.grid_propagate(False)  # Empêche le cadre de se redimensionner en fonction du contenu
        ...
        pt = Table(inner_frame, dataframe=df)
        pt.show()
...
def update_table():
    ...
    if df is not None and excel_file_path is not None:
...
#Fonction qui ajoute les checkbutton qui sont dans le cadre6
def add_checkbutton():
...
#Fonction qui enregistre le tableau du cadre7 
def save_file():
    global df
...
#Cette fonction enregistre automatiquement le fichier excel des que l'utilisateur clique sur le bouton show classification dans le cadre8 
def saveAuto(current_excel_file):
    global df
    # Vérifier si le DataFrame est vide
    if df.empty:
        messagebox.showerror("Erreur", "Le DataFrame est vide.")
        return
    # Vérifier si le fichier Excel courant a été spécifié
    if current_excel_file is None:
        messagebox.showerror("Erreur", "Aucun fichier Excel courant n'a été spécifié.")
        return
    # Mettre à jour le fichier Excel courant avec le contenu du tableau du cadre7
    df_copy = df.copy()
    df_copy.iloc[0, :] = ""
    df_copy = df_copy[df_copy.astype(bool).any(axis=1)]
    df_copy.to_excel(current_excel_file, index=False)
    messagebox.showinfo("Sauvegarde réussie", "Les données ont été sauvegardées avec succès.")
...
current_excel_file = create_default_excel_file(dossier)


def classification():
    global cadre8, cadre4, cadre7
    saveAuto(current_excel_file)  # Sauvegarde automatique de current_excel_file
   # Obtenir les noms d'images dans le cadre4
    listbox_items = []
    for button in filebuttons:
        filename = button['text']
        name_without_extension = os.path.splitext(filename)[0]
        listbox_items.append(name_without_extension)
    # Obtenir les noms de fichiers dans le tableau du cadre7
    tableau_items = df["Element"].tolist()
    if tableau_items[-1] == "couleur":
        tableau_items = tableau_items[:-1]
    # Vérification des noms d'images et les noms de fichiers sont différents
    if set(listbox_items) != set(tableau_items):
        messagebox.showerror("Erreur", "Les noms d'images sont différents des noms de fichiers")
        return
    # Effacer le contenu existant du cadre8
    for widget in cadre8.winfo_children():
        widget.destroy()
    try:
        cm = createObject(dossier, extension, current_excel_file)
        cm.load_table()
        cm.datacube_creation()
        cm.mineralcube_creation()
        cm.Minerals
        cm.plot_mineral_mask(cadre8)
        # Le cadre8 ne change pas de dimension
        cadre8.grid_propagate(False)
        if not os.path.isfile(current_excel_file):
            print("Aucun fichier n'a été spécifié.")
        return
